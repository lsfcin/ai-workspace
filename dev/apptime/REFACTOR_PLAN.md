...
- [ ] **R4a. Split `analytics_screen.dart` (2042 lines) into 5 files.**
...
- [ ] **R4b. Split `insights_screen.dart` (947 lines) into 3 files.**
...
- [ ] **R4c. Split `MonitoringService.kt` (619 lines) into 3 files.**
...
- [ ] **R4d. Extract `OverlayService.kt` feedback engine (557 lines).**
...
- [ ] **R12. Remove `_labelForApp()` wrapper in `analytics_screen.dart`.**
...
- [ ] **R13. Cache SharedPreferences instance in `OverlayService`.**
...
- [ ] **R20. Remove `dailyGoalMinutes` from `StorageService`.**
...
- [ ] **R25. Add `canLaunchUrl` guard in MonitoringScreen's `_InsightCard`.**
...
- [ ] **R26. Fix `_setLocale(null)` comment vs implementation.**
...
- [ ] **R31. Localise `_sevenDayLabel()` weekday abbreviations in `analytics_screen.dart`.**
  Currently hardcoded Portuguese: `['seg', 'ter', 'qua', ...]`. Should derive from `AppLocalizations` or Dart's `intl` package.

- [ ] **R32. Localise the `'2h ideal'` / `'4h crítico'` reference-line labels in `_UsageTrend30d`.**
  Hardcoded Portuguese strings bypassing `AppLocalizations`.

- [ ] **R33. Migrate `_classificationMessage()` in `analytics_screen.dart` through `AppLocalizations`.**
...
- [ ] **R36. Document `migrateCorruptedDeviceDaily()` run-every-start behavior.**
...
- [ ] **R37. Document `REOPEN_TOLERANCE_MS = 120_000L` tradeoff in `MonitoringService.kt`.**
